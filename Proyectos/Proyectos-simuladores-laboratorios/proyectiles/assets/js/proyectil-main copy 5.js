 ...
 * Nivel 6: Asegura la máxima tasa de refresco y optimización del dibujo.
 */
...
let dataPanelContent = null;

function init() {
  canvas = document.getElementById(CANVAS_ID);
  ctx = canvas.getContext('2d');

  // --- ¡NUEVO! Configuración de alta calidad del canvas ---
  // Esto intenta usar la resolución física de la pantalla (Retina) para un dibujo más nítido.
  const dpr = window.devicePixelRatio || 1;
  canvas.style.width = window.innerWidth + 'px';
  canvas.style.height = window.innerHeight + 'px';
  canvas.width = window.innerWidth * dpr;
  canvas.height = window.innerHeight * dpr;
  ctx.scale(dpr, dpr); // Escala todos los dibujos para que coincidan
...
function resizeCanvas() {
  // --- ¡ACTUALIZADO! Resizing para pantallas de alta resolución ---
  const dpr = window.devicePixelRatio || 1;
  canvas.style.width = window.innerWidth + 'px';
  canvas.style.height = window.innerHeight + 'px';
  canvas.width = window.innerWidth * dpr;
  canvas.height = window.innerHeight * dpr;
  ctx.scale(dpr, dpr); // Vuelve a aplicar la escala en cada resize
}
...
function draw() {
  ...
  // ¡NUEVO! Ya no necesitamos dibujar un fondo sólido con ctx.fillRect
  // El GIF ya está detrás y lo vemos a través del canvas transparente.
